...
bool data_display::test_result(void)
{
    ...
    derivates[0]=m_vec_deriv[int(numbof_values/2)].y;
    derivates[1]=m_vec_deriv[int(numbof_values/2+1)].y;
    derivates[2]=m_vec_deriv[int(numbof_values*3/4)].y;
    ...
    if( isnan( derivates[2] ) ) return false;//not a number

    cout<<"Derivative: "<<derivates[2]<<endl;
    return true;//hit
}
...
bool data_display::generate_data(void)
{
    ...
    for(float t=m_time_start; t<=m_time_end; t+=m_step_size)
    {
        ...
        //add derivative for reporter protein (tetR)
        temp_point.x=t;
        temp_point.y=m_deriv_prot_tetR;
        m_vec_deriv.push_back(temp_point);
